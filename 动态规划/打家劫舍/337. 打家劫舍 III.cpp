...
public:
    ...
    int dp(TreeNode* root,int count){
        ...
        int sum = root->val;
        if(root->left){
            sum += dp(root->left->left,count+2)+dp(root->left->right,count+2);
        }
        if(root->right)
            sum += dp(root->right->left,count+2)+dp(root->right->right,count+2);
        int sum2 = dp(root->left,count+1)+dp(root->right,count+1);
        mp[root] = sum>sum2?sum:sum2;
        cout<<"root"<<count<<" "<<root->val<<" "<<mp[root]<<endl;;
